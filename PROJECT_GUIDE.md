...
### 坑 31：离屏窗口缩略图截断
窗口部分拖出屏幕时，`CGSHWCaptureWindowList`/`CGWindowListCreateImage` 只返回可见部分，缩略图被截断。**解决：已知限制，不干预。macOS WindowServer 不渲染屏外像素，透明填充会引入额外复杂度，暂不做处理**。
...
> **最后更新**：2026-07-08（新增坑 37-44：Key 不匹配、Overlay 阻塞热区、动画 Timer 覆盖、两步调用卡顿、坐标系混用、ObservableObject 缺失、Sparkle 英文、更新状态初始 nil；3.2/3.3 更新为统一 handleDragStage + 动画 Timer 管理；目录结构 + 功能表新增触控反馈/HapticFeedback/UpdaterService）